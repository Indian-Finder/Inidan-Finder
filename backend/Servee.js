 ...
 * Required when using secure cookies behind a reverse proxy, otherwise sessions won't persist
 * and OAuth will fail with "Invalid state".
 */
...
// -------------------- Config --------------------
// Optional: if your frontend is hosted on a different domain (Netlify, etc.)
...
// Cookies:
// - For OAuth redirects, SameSite=Lax is safest.
// - If you later need cross-site XHR with credentials from a different domain, use SameSite=None + Secure=true.
...
// Prefer X_REDIRECT_URI; accept common alternates too
const X_REDIRECT_URI =
  process.env.X_REDIRECT_URI ||
  process.env.X_REDIRECT_URL || // (common typo)
  process.env.TWITTER_CALLBACK_URL ||
  process.env.TWITTER_CALLBACK_URI;

// Where to send the user after successful OAuth
...
async function ensureXToken(req) {
  ...
  if (!token) {
    const err = new Error("Not connected to X. Use /api/x/connect first.");
    err.status = 401;
    throw err;
  }
  return token;
...
app.get("/api/status", (req, res) => {
  res.json({
    ...
    hasXConfig: Boolean(X_CLIENT_ID && X_CLIENT_SECRET && X_REDIRECT_URI),
...
app.get("/", (req, res) => {
  ...
  res.type("text").send("Backend running. Try /api/status");
});
...
app.get("/api/twitter/login", (req, res) => {
  try {
    ...
    // ✅ IMPORTANT: ensure session is saved BEFORE redirecting to X
