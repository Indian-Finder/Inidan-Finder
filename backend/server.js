...
app.post("/api/upload", (req, res) => {
  // Accept multipart uploads from various frontends.
  // Some clients send the file field as "file", others as "media" or "upload".
  // We'll accept any field name and use the first file we receive.
    ...
    try {
      ...
      if (!file) {
        return res.status(400).json({
          ok: false,
          error: "No file uploaded. Expected multipart/form-data with a file field (e.g. file/media/upload).",
        });
      }
      ...
      // Multer may store in temp; move into our uploads dir
      fs.renameSync(file.path, dest);

      const mediaType = isVideoExt(ext) ? "video" : "image";
      const item = {
        id,
